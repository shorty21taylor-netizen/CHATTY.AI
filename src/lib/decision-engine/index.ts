...
import type { DecisionTrace, UnifiedContext } from "../db/types";
...
export async function runDecisionEngine(orgId: string, options: { vertical?: string; operatorName?: string; date?: string } = {}): Promise<DecisionRunResult> {
  ...
  const context = (await getOrCreateContext(orgId, date)) as unknown as UnifiedContext;
  const recentEvents = await getRecentEvents(orgId, 100);
  const previousBriefs = (await getRecentBriefs(orgId, 7)) as any[];
  ...
  const updatedContext = (await getOrCreateContext(orgId, date)) as unknown as UnifiedContext;
  ...
  const trace: DecisionTrace = { pass1: {...pass1,timestamp:pass1.timestamp}, pass2: {...pass2,timestamp:pass2.timestamp}, pass3: {...pass3,timestamp:pass3.timestamp}, total_duration_ms: totalDurationMs } as unknown as DecisionTrace;
  const brief = (await insertBrief(orgId, { brief_date: date, context_id: context.id, decision_engine_trace: trace, recommendations: pass3.recommendations, priority: pass3.priority, voice_summary: pass3.voice_text })) as any;
